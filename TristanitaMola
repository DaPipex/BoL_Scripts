...
function Menu()
	...
	--TristyMenu.killsteal:addParam("Gks", "Killsteal on/off", SCRIPT_PARAM_ONOFF, true)
...
function OnTick()
	...
	ActualizarRangos()
	Killsteal()
	if TristyMenu.keys.ComboKey then Combo() end
...
function CastSpecialW()
	...
	if VPredActive then
		...
		else local Position = freePredW:GetPrediction(Target)
